...
class CPU extends Module {
  ...
  if_id.io.in.noflush := true.B
  id_ex_ctrl.io.in.noflush := true.B
  ex_mem_ctrl.io.in.noflush := true.B
  mem_wb_ctrl.io.in.noflush := true.B
  ...
  hazard.io.ID_EX_memAccess := id_ex_ctrl.io.data.mem_ctrl.memRead || id_ex_ctrl.io.data.mem_ctrl.memWrite
  ...
  val old_dmem_writeData = RegNext(ex_mem.io.data.regRData2)
  val old_dmem_memRead   = RegNext(ex_mem_ctrl.io.data.mem_ctrl.memRead)
  val old_dmem_memWrite  = RegNext(ex_mem_ctrl.io.data.mem_ctrl.memWrite)
  val old_dmem_maskMode  = RegNext(ex_mem_ctrl.io.data.mem_ctrl.maskMode)
  val old_dmem_sext      = RegNext(ex_mem_ctrl.io.data.mem_ctrl.sext)
  val old_dmem_readData  = RegNext(io.dmem.readData)

  // Set data.mem_ctrlemory IO (line 71 of single-cycle/cpu.scala)
  io.dmem.addr      := Mux(old_dmem_sb_sh, old_dmem_addr,     Mux(dmem_sb_sh, old_dmem_addr     , ex_mem.io.data.result))
  io.dmem.writeData := Mux(old_dmem_sb_sh, old_dmem_readData, Mux(dmem_sb_sh, old_dmem_writeData, ex_mem.io.data.regRData2))
  io.dmem.memRead   := Mux(old_dmem_sb_sh, false.B,           Mux(dmem_sb_sh, true.B            , ex_mem_ctrl.io.data.mem_ctrl.memRead))
  io.dmem.memWrite  := Mux(old_dmem_sb_sh, old_dmem_memWrite, Mux(dmem_sb_sh, false.B           , ex_mem_ctrl.io.data.mem_ctrl.memWrite))
  io.dmem.maskMode  := Mux(old_dmem_sb_sh, old_dmem_maskMode, Mux(dmem_sb_sh, "b10".U           , ex_mem_ctrl.io.data.mem_ctrl.maskMode))
  io.dmem.sext      := Mux(old_dmem_sb_sh, old_dmem_sext,     Mux(dmem_sb_sh, 0.U               , ex_mem_ctrl.io.data.mem_ctrl.sext))
  io.dmem.readBack       := old_dmem_readData
